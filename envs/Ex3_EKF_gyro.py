...
class Ex3_EKF_gyro(gym.Env):
    ...
    def step(self, action):  # here u1,u2=measurement, which is a result of the action
        ...
        if self.realMeasurement:
            ...
            acc_m_q = np.dot(np.dot(quatLeftMulMat(quatConj(self.q_t)), quatRightMulMat(self.q_t)),
                             quatPure2Q([0, 0, 1]))
            if not self.clean:
    ...
    def reset(self, eval=False):
        ...
        if self.realMeasurement:
            ...
            if not eval:
                self.q_t = np.random.uniform([-1, -1, -1, -1],
                                             [1, 1, 1, 1])  # the state q, the initial value can be set randomly
...
if __name__ == '__main__':
    ...
    if env.saveChoice(choice) == 'saveData':
        ...
        for i in range(int(T / env.dt)):
            ...
            # path.append(s)
            omega_obs, acc_m, mag_m, q_t, q_gyro, r, info, done = env.step(
